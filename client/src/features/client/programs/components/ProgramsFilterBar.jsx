...
export default function ProgramsFilterBar({ activeCategory, onChange }) {
  return (
    <section className="mb-12 bg-base-200 px-4 py-6 md:px-8">
      <div className="container flex flex-wrap gap-3 px-0">
        {categories.map((cat) => (
          <button
            key={cat.id}
            onClick={() => onChange(cat.id)}
            className={`font-display rounded-lg px-6 py-2 text-sm font-bold uppercase tracking-tight transition-colors ${
